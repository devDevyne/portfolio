...
export default async function WorkLayout({ children }: { children: ReactNode }) {
  ...
  const projects = experience ? await getProjects() : [];
  return (
    <div className="grid gap-9 py-8 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-14 lg:py-12">
      <WorkSidebar experience={experience ? { company_name: experience.company_name, period: experience.period } : null} projects={projects.map(({ meta }) => ({ slug: meta.slug, title: meta.title }))} />
